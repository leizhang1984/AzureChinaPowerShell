#Created By MSFT leizhang (leizha@microsoft.com) on 2021-06-10

##################################VERY IMPORTANT##############################
#PLEASE RUN in PowerShell 7.0
##############################################################################
...
#foreach ($rows in $p) 
{
        ...
        {  
		    ...
		    $VMUserName = $_.VMUsername.Trim()
		    $VMPassword = $_.VMPassword.Trim()
		
		    $vmRG = $_.VMRG.Trim()
		    $diagStorageAccountName = $_.DiagStorageAccountName.Trim()
		    $vNetName = $_.VirtualNetworkName.Trim()
		    $vNetRG = $_.VNetRG.Trim()
		    $subnetName = $_.SubnetName.Trim()
			
		    $osDiskSKU = $_.OSDiskSKU.Trim()
		    $osDiskSizeInGB = $_.OSDiskSizeInGB.Trim()
		    $dataDiskSKU = $_.DataDiskSKU.Trim()
		    $dataDiskSizeInGB = $_.DataDiskSizeInGB.Trim()
		    $vmAvailabilitySetName = $_.VMAvailabilitySetName.Trim()
		    ...
		    $nic1 = New-AzNetworkInterface -Name $nicname -ResourceGroupName $rg.ResourceGroupName -Location $location -SubnetId $subnet.Id -Force
