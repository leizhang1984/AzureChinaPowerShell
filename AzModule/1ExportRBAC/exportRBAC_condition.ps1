...
foreach ($sub in $SubscriptionNames)
{
        ...
        foreach ($datarow in $exports)
        {
                ...
                $output | add-member -Membertype NoteProperty -Name "RoleAssignmentId" -value $datarow.RoleAssignmentId
                ...
                $output | add-member -Membertype NoteProperty -Name "RoleDefinitionId" -value $datarow.RoleDefinitionId
                ...
                $output | add-member -Membertype NoteProperty -Name "CanDelegate" -value $datarow.CanDelegate

                $output | add-member -Membertype NoteProperty -Name "Description" -value $datarow.Description
                $output | add-member -Membertype NoteProperty -Name "ConditionVersion" -value $datarow.ConditionVersion
                $output | add-member -Membertype NoteProperty -Name "Condition" -value $datarow.Condition
...
$csvpath = $pwd.Path + "\export_rbac_condition.csv"
$logArray | convertto-Csv -NoTypeInformation | out-file $csvpath -append -Encoding utf8 
Write-Output "Export RBAC with condition successfully"